...
func NewDNSProviderByValues(values map[string]string) (*DNSProvider, error) {
	if values[envPrivKey] == "" || values[EnvTenancyOCID] == "" || values[EnvUserOCID] == "" || values[EnvPubKeyFingerprint] == "" || values[EnvRegion] == "" || values[EnvCompartmentOCID] == "" {
		return nil, fmt.Errorf("oraclecloud: OCI_PRIVKEY or OCI_TENANCY_OCID or OCI_USER_OCID or OCI_PUBKEY_FINGERPRINT or OCI_REGION or OCI_COMPARTMENT_OCID")
	}
